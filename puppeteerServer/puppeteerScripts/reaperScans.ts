...
export async function getManga(
  ...
  try {
    ...
    if (!chapResponse.ok)
      throw new Error('Manga: Unable to get Chapter List. Contact an admin or try again later!');
    ...
    if (chapterSlugs.length == 0 || chapterSlugs.length != chapterText.length)
      throw new Error('Manga: Issue fetching Chapters');

    const title = await page.evaluate(() => document.querySelector('h2.font-semibold')?.innerHTML, {
      timeout: 500,
    });
    ...
    if (icon || inputDate < oneMonthAgo) {
      ...
      const photo = (await photoSelect?.evaluate((el) => el.getAttribute('src'))).replace(
        'w=96',
        'w=384'
      );
    ...
    const currIndex = chapterSlugs.indexOf(
      url.replace(`https://reaperscans.com/series/${seriesSlug}/`, '')
    );

    if (currIndex == -1 && !ignoreIndex) {
      throw new Error(
        'Manga: unable to find current chapter. Try opening page and copying URL or contact an Admin!'
      );
    }
