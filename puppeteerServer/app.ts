        ...
        try {
            ...
            if ((job.returnvalue.iconBuffer) || (job.returnvalue.slugList && job.returnvalue.slugList != job.data.oldSlugList)) {
                ...
                if (newChapterCount != 0)
                        batch.batchData.newChapterCount += newChapterCount
...
async function sendUpdate(batch: updateCollector) {
    ...
    if (batch.batchData.newData.length > 0) {
        if (!config.updateSettings.refetchImgs) { // returns all manga together if images not fetched
            ...
            else if (config.updateSettings.autoUpdateInfo) console.log(`${batch.batchData.newData.length} / ${batch.batchData.completedCount} Manga Update Saved With ${batch.batchData.newChapterCount} New Chapters!`)

        } else { //send each manga sepratly due to images
...
if (config.updateSettings.autoUpdateInfo) setInterval(updateAllManga, config.updateSettings.updateDelay)