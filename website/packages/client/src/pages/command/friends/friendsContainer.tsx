...
async function fetchCount(): Promise<{message: string, data: {friendCount:number, incomingCount:number, outgoingCount:number}}> {
  ...
  return (
    <Box>
      <Tabs value={value} onChange={handleChange} centered variant="fullWidth" TabIndicatorProps={{ style: { display: 'none' } }}>
        <Tab icon={<Badge badgeContent={data?.data.friendCount} color="primary"><GroupIcon/></Badge>} label="Friends"/>
        <Tab icon={<Badge badgeContent={data?.data.incomingCount} color="primary"><ArchiveIcon/></Badge>} label="Incomming"/>  
        <Tab icon={<Badge badgeContent={data?.data.outgoingCount}color="primary"><UnarchiveIcon/></Badge>} label="Outgoing"/>
      </Tabs>
